...
const Men = () => {
  ...
  const maleClothes = clothesData.filter(
    (clothes) => clothes.category === "Male"
  );
  const filteredClothes = maleClothes.filter((clothes) => {
    if (selectedCategory === "All") {
      return true; // Show all clothes
    } else {
      return (
        clothes.category === selectedCategory ||
        clothes.type === selectedCategory
      );
    }
  });
    ...
    <div className="clothes">
      ...
      <div className="clothes-wrapper">
        <div className="w-full filter-container gap-5 items-center">
          ...
          <button
            className={`${
              selectedCategory === "Pants & Joggers"
                ? "bg-black text-white"
                : "bg-white text-black"
            } px-3 py-1 rounded-[3px] border border-black`}
            onClick={() => setSelectedCategory("Pants & Joggers")}
          >
            Pants & Joggers
          </button>
